class SharesController < ApplicationController
	...
	before_action :require_shareable_permissions!, only: [:create, :new, :destroy]
	...
	def destroy
		if user_password.deletable?
			redirect_to @password, alert: "Owner cannot remove themselves"
			return
		end
		@password.user_passwords.where(user_id: params[:id]).destroy_all
		redirect_to @password, notice: "User access removed"
	end
